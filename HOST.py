...
def env_int(name, default=None, required=False):
    ...
    try:
        ...
        raise SystemExit(f"❌ {name} must be integer, got: {val}")

TOKEN = os.getenv("BOT_TOKEN")
if not TOKEN:
    raise SystemExit("❌ BOT_TOKEN env missing. Railway Variables me BOT_TOKEN set karo.")

OWNER_ID = env_int("OWNER_ID", required=True)
ADMIN_ID = env_int("ADMIN_ID", OWNER_ID)
YOUR_USERNAME = os.getenv("YOUR_USERNAME", "@GAURAV_BHAI1")
UPDATE_CHANNEL = os.getenv("UPDATE_CHANNEL", "https://t.me/DESTROYDDOSLODER")

BASE_DIR = os.path.abspath(os.path.dirname(__file__))
UPLOAD_BOTS_DIR = os.getenv("UPLOAD_BOTS_DIR", os.path.join(BASE_DIR, "upload_bots"))
IROTECH_DIR = os.getenv("IROTECH_DIR", os.path.join(BASE_DIR, "inf"))
DATABASE_PATH = os.getenv("DATABASE_PATH", os.path.join(IROTECH_DIR, "bot_data.db"))

# File upload limits
FREE_USER_LIMIT = env_int("FREE_USER_LIMIT", 2)
SUBSCRIBED_USER_LIMIT = env_int("SUBSCRIBED_USER_LIMIT", 15)
ADMIN_LIMIT = env_int("ADMIN_LIMIT", 999)
    ...
    try:
        ...
        for user_id, expiry in c.fetchall():
            try:
                ...
                logger.warning(f"⚠️ Invalid expiry for {user_id}: {expiry}")
        ...
        try:
            ...
            if not is_running:
                ...
                if 'log_file' in script_info and hasattr(script_info['log_file'], 'close') and not script_info['log_file'].closed:
                    ...
                    except Exception as log_e: logger.error(f"Error closing log {script_key}: {log_e}")
            ...
            logger.error(f"Error checking process {script_key}: {e}", exc_info=True)
            return False
    return False
    ...
    try:
        if 'log_file' in process_info and hasattr(process_info['log_file'], 'close') and not process_info['log_file'].closed:
            try:
                ...
                logger.error(f"Error closing log for {script_key}: {log_e}")
        ...
        if process and hasattr(process, 'pid'):
            ...
            if pid:
                try:
                    ...
                    logger.info(f"Killing tree {script_key} (PID: {pid})")
        ...
        logger.error(f"❌ Error killing tree {pid} ({script_key}): {e}", exc_info=True)
...
TELEGRAM_MODULES = {
    ...
    'psutil': 'psutil',
    # Core modules (should not install)
...
def run_script(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply, attempt=1):
    ...
    try:
            ...
            try:
                ...
                if return_code != 0 and stderr:
                    ...
                    if match_py:
                        ...
                        bot.reply_to(message_obj_for_reply, f"❌ Pre-check error '{file_name}':\n```\n{error_summary}\n```", parse_mode='Markdown')
                        return
                ...
                bot.reply_to(message_obj_for_reply, f"❌ Pre-check error '{file_name}': {e}")
                return
            finally:
        ...
        try:
            ...
            if log_file and not log_file.closed: log_file.close()
            bot.reply_to(message_obj_for_reply, f"❌ Error starting: {str(e)}")
...
def save_user_file(user_id, file_name, file_type='py'):
    with DB_LOCK:
        ...
        try:
            ...
            user_files[user_id].append((file_name, file_type))
        except sqlite3.Error as e: logger.error(f"❌ SQLite save: {e}")
        except Exception as e: logger.error(f"❌ Save error: {e}", exc_info=True)
        finally: conn.close()
...
def remove_user_file_db(user_id, file_name):
    with DB_LOCK:
        ...
        except sqlite3.Error as e: logger.error(f"❌ SQLite remove: {e}")
        finally: conn.close()
...
def add_active_user(user_id):
    ...
    with DB_LOCK:
        ...
        try:
            ...
            conn.commit()
        except sqlite3.Error as e: logger.error(f"❌ SQLite active: {e}")
        finally: conn.close()
...
def save_subscription(user_id, expiry):
    with DB_LOCK:
        ...
        except sqlite3.Error as e: logger.error(f"❌ SQLite sub save: {e}")
        finally: conn.close()
...
def remove_subscription_db(user_id):
    with DB_LOCK:
        ...
        except sqlite3.Error as e: logger.error(f"❌ SQLite sub remove: {e}")
        finally: conn.close()
...
def add_admin_db(admin_id):
    with DB_LOCK:
        ...
        except sqlite3.Error as e: logger.error(f"❌ SQLite admin add: {e}")
        finally: conn.close()


def remove_admin_db(admin_id):
    if admin_id == OWNER_ID:
        logger.warning("Cannot remove OWNER_ID.")
        return False
    with DB_LOCK:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()
        ...
        try:
            ...
            logger.error(f"❌ SQLite admin remove: {e}")
            return False
        finally: conn.close()
    ...
    try:
        ...
        if req_file:
            ...
            try:
                ...
                error_msg = f"❌ Failed Python deps:\n```\n{e.stderr or e.stdout}\n```"
                if len(error_msg) > 4000: error_msg = error_msg[:4000]
...
def _logic_send_welcome(message):
    ...
    if user_id not in active_users:
        ...
        except Exception as e: logger.error(f"⚠️ Owner notify failed: {e}")
    ...
    try:
        ...
        logger.error(f"Welcome send error: {e}", exc_info=True)
...
def _logic_bot_speed(message):
    ...
    try:
        ...
        logger.error(f"Speed error: {e}", exc_info=True)
...
def _logic_statistics(message):
    ...
    for script_key_iter, script_info_iter in list(bot_scripts.items()):
        try:
            ...
            if is_bot_running(int(s_owner_id_str), script_info_iter['file_name']):
                running_bots_count += 1
                if int(s_owner_id_str) == user_id:
                    user_running_bots += 1
        except Exception:
            continue
...
def _logic_run_all_scripts(message_or_call):
    ...
    summary_msg = (f"✅ All Users' Scripts - Complete:\n\n"
    ...
    logger.info(f"Run all scripts finished. Started: {started_count}. Skipped: {skipped_files}")
...
def start_bot_callback(call):
    try:
        ...
        bot.answer_callback_query(call.id, "Error: Invalid start.", show_alert=True)
    except Exception as e:
        logger.error(f"Error: {e}", exc_info=True)
...
def stop_bot_callback(call):
    try:
        ...
        logger.error(f"Parse error: {e}")
        bot.answer_callback_query(call.id, "Error: Invalid stop.", show_alert=True)
    except Exception as e:
        logger.error(f"Error: {e}", exc_info=True)
...
def restart_bot_callback(call):
    try:
        ...
        logger.error(f"Parse error: {e}")
        bot.answer_callback_query(call.id, "Error: Invalid restart.", show_alert=True)
    except Exception as e:
        logger.error(f"Error: {e}", exc_info=True)
...
def delete_bot_callback(call):
    try:
        ...
        if os.path.exists(file_path):
            ...
            except OSError as e: logger.error(f"Error: {e}")
        ...
        logger.error(f"Parse error: {e}")
        bot.answer_callback_query(call.id, "Error: Invalid delete.", show_alert=True)
    except Exception as e:
        logger.error(f"Error: {e}", exc_info=True)
...
def logs_bot_callback(call):
    try:
        ...
        logger.error(f"Parse error: {e}")
        bot.answer_callback_query(call.id, "Error: Invalid logs.", show_alert=True)
    except Exception as e:
        logger.error(f"Error: {e}", exc_info=True)
...
def unlock_bot_callback(call):
    ...
    logger.warning(f"Bot unlocked by {call.from_user.id}")
...
def process_broadcast_message(message):
    ...
    has_media = bool(message.photo or message.video or message.document or message.sticker or message.voice or message.audio)
    if not broadcast_content and not has_media:
...
def handle_confirm_broadcast(call):
    ...
    try:
        ...
        if not original_message: raise ValueError("Cannot retrieve original message.")
...
def process_add_admin_id(message):
    ...
    try:
        ...
        if new_admin_id <= 0: raise ValueError("Must be positive")
        ...
        logger.warning(f"Admin {new_admin_id} added by {message.from_user.id}")
        ...
        bot.register_next_step_handler(msg, process_add_admin_id)
    except Exception as e: logger.error(f"Error: {e}", exc_info=True); bot.reply_to(message, "Error.")
...
def process_remove_admin_id(message):
    ...
    try:
        ...
        if admin_id_remove <= 0: raise ValueError("Must be positive")
        if admin_id_remove == OWNER_ID: bot.reply_to(message, "⚠️ Cannot remove self."); return
        if admin_id_remove not in admin_ids: bot.reply_to(message, f"⚠️ Not Admin."); return
        if remove_admin_db(admin_id_remove):
            logger.warning(f"Admin {admin_id_remove} removed by {message.from_user.id}")
...
def process_add_subscription_details(message):
    ...
    try:
        ...
        logger.info(f"Sub for {sub_user_id} by {message.from_user.id}. Expiry: {new_expiry:%Y-%m-%d}")
...
def process_remove_subscription_id(message):
    ...
    try:
        ...
        if sub_user_id_remove <= 0: raise ValueError("Must be positive")
        if sub_user_id_remove not in user_subscriptions:
            bot.reply_to(message, f"⚠️ User has no active sub."); return
        remove_subscription_db(sub_user_id_remove)
        logger.warning(f"Sub removed for {sub_user_id_remove} by {message.from_user.id}")
...
def process_check_subscription_id(message):
    ...
    try:
        ...
        if sub_user_id_check <= 0: raise ValueError("Must be positive")
        if sub_user_id_check in user_subscriptions:
            ...
            if expiry_dt:
                if expiry_dt > datetime.now():
                    ...
                    bot.reply_to(message, f"⚠️ User expired on {expiry_dt:%Y-%m-%d %H:%M:%S}.")
                    remove_subscription_db(sub_user_id_check)
            else: bot.reply_to(message, f"⚠️ Expiry missing.")
        else: bot.reply_to(message, f"ℹ️ No active sub.")
    except ValueError:
        bot.reply_to(message, "⚠️ Invalid ID.")
        msg = bot.send_message(message.chat.id, "💳 Enter User ID or /cancel.")
        bot.register_next_step_handler(msg, process_check_subscription_id)
...
if __name__ == '__main__':
    ...
    while True:
        try:
            ...
            logger.warning("Polling ReadTimeout. Restarting in 5s..."); time.sleep(5)
            ...
            logger.info("Restarting in 30s..."); time.sleep(30)
        finally:
            logger.warning("Polling attempt finished.")
            time.sleep(1)